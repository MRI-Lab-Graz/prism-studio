...
def check_is_ascii(string: str) -> None:
    ...
    if not all(ord(x) > 32 and ord(x) < 127 for x in string):
        warnings.warn(
            "Invalid char: header entries should contain only ASCII"
            ' characters and no spaces: "{}"'.format(string)
        )


def check_signal_header_correct(
    channels: List[Dict[str, Union[str, float, None]]], i: int, file_type: int
) -> None:
    """
    ...
    if len(ch["label"]) > 16:  # type: ignore
        warnings.warn(
            "Label of channel {} is longer than 16 ASCII chars. "
    ...
    dmin, dmax = (
        (-8388608, 8388607)
        if file_type in (FILETYPE_BDFPLUS, FILETYPE_BDF)
        else (-32768, 32767)
    )
    if ch["digital_min"] < dmin:  # type: ignore
        raise ValueError(
            "Digital minimum for channel {} ({}) is {}, "
            "but minimum allowed value is {}".format(i, label, ch["digital_min"], dmin)
        )
    if ch["digital_max"] > dmax:  # type: ignore
        raise ValueError(
            "Digital maximum for channel {} ({}) is {}, "
            "but maximum allowed value is {}".format(i, label, ch["digital_max"], dmax)
        )
    ...
    if len(str(ch["physical_min"])) > 8 and ch["physical_min"] < -99999999:  # type: ignore
        raise ValueError(
            ...
            "please convert the signals to another dimension (eg uV to mV)".format(
    ...
    if len(str(ch["physical_max"])) > 8 and ch["physical_max"] > 99999999:  # type: ignore
        raise ValueError(
            ...
            "however, EDF+ can only save 8 chars, critical precision loss is expected, "
            "please convert the signals to another dimension (eg uV to mV).".format(
                i, label, ch["physical_max"], len(str(ch["physical_max"]))
            )
        )
    ...
    if len(str(ch["physical_min"])) > 8:
        warnings.warn(
            ...
            "some loss of precision is to be expected".format(
                i,
                label,
                ch["physical_min"],
    ...
    if len(str(ch["physical_max"])) > 8:
        warnings.warn(
            "Physical maximum for channel {} ({}) is {}, which has {} chars, "
            "however, EDF+ can only save 8 chars, will be truncated to {}, "
            "some loss of precision is to be expected.".format(
                i,
                label,
...
def gender2int(gender: Union[int, str, None]) -> Optional[int]:
    warnings.warn(
        "Function 'gender2int' is deprecated, use 'sex2int' instead.",
        DeprecationWarning,
        stacklevel=2,
    )
...
class EdfWriter:
    ...
    def __init__(
        self, file_name: str, n_channels: int, file_type: int = FILETYPE_EDFPLUS
    ) -> None:
        ...
        self.number_of_annotations = (
            1 if file_type in [FILETYPE_EDFPLUS, FILETYPE_BDFPLUS] else 0
        )
    ...
    def update_header(self) -> None:
        ...
        patient_ident = (
            len(self.patient_code)
            + len(self.patient_name)
            + len(self.patient_additional)
            + 3
            + 1
            + 11
        )  # 3 spaces 1 sex 11 birthdate
        record_ident = (
            len(self.equipment)
            + len(self.technician)
            + len(self.admincode)
            + len(self.recording_additional)
            + len("Startdate")
            + 3
            + 11
        )  # 3 spaces 11 birthdate

        if patient_ident > 80:
            warnings.warn(
                "Patient code, name, sex and birthdate combined must not be larger than 80 chars. "
        ...
        for ch in self.channels:
            ...
            if "sample_rate" in ch:
                raise FutureWarning(
                    "Use of `sample_rate` is deprecated, use `sample_frequency` instead"
                )
        ...
        set_startdatetime(
            self.handle,
            self.recording_start_time.year,
            self.recording_start_time.month,
            self.recording_start_time.day,
            self.recording_start_time.hour,
            self.recording_start_time.minute,
            self.recording_start_time.second,
        )
        # subseconds are noted in nanoseconds, so we multiply by 100
        if self.recording_start_time.microsecond > 0:
            set_starttime_subsecond(
                self.handle, self.recording_start_time.microsecond * 100
            )
    ...
    def setSignalHeader(
        self, edfsignal: int, channel_info: Dict[str, Union[str, int, float, None]]
    ) -> None:
        """
    ...
    def setSignalHeaders(
        self, signalHeaders: List[Dict[str, Union[str, int, float, None]]]
    ) -> None:
        """
    ...
    def setGender(self, gender: Union[int, str, None]) -> None:
        warnings.warn(
            "Function 'setGender' is deprecated, use 'setSex' instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def setDatarecordDuration(self, record_duration: Union[float, int]) -> None:
        ...
        """
        warnings.warn(
            "Forcing a specific record_duration might alter calculated sample_frequencies when reading the file"
        )
    ...
    def setStartdatetime(self, recording_start_time: Union[datetime, str]) -> None:
        ...
        if not isinstance(recording_start_time, datetime):
            recording_start_time = datetime.strptime(
                recording_start_time, "%d %b %Y %H:%M:%S"
            )
    ...
    def setSamplefrequency(
        self, edfsignal: int, samplefrequency: Union[int, float]
    ) -> None:
        """
    ...
    def setPhysicalMaximum(
        self, edfsignal: int, physical_maximum: Union[int, float]
    ) -> None:
        """
    ...
    def setPhysicalMinimum(
        self, edfsignal: int, physical_minimum: Union[int, float]
    ) -> None:
        """
    ...
    def writeSamples(
        self, data_list: Union[List[np.ndarray], np.ndarray], digital: bool = False
    ) -> None:
        """
        ...
        if not all(s.flags.c_contiguous for s in data_list):
            warnings.warn(
                "signals are in Fortran order. Will automatically "
        ...
        for chan in self.channels:
            assert chan["physical_min"] != chan["physical_max"], (
                ...
                "physical_max {}".format(
                    chan["label"], chan["physical_min"], chan["physical_max"]
                )
            )
        ...
        while notAtEnd:
            ...
            dataRecord = np.array([], dtype=np.int32 if digital else None)
            for i in np.arange(len(data_list)):
                dataRecord = np.append(
                    dataRecord,
                    data_list[i][int(ind[i]) : int(ind[i] + smp_per_record[i])],
                )
            ...
            if success < 0:
                raise OSError(
                    f"Unknown error while calling blockWriteSamples: {success}"
                )
        ...
        for i in np.arange(len(data_list)):
            lastSamples = np.zeros(
                smp_per_record[i], dtype=np.int32 if digital else None
            )
            ...
            if lastSampleInd > 0:
                ...
                if success < 0:
                    raise OSError(
                        f"Unknown error while calling writeSamples: {success}"
                    )

    def writeAnnotation(
        self,
        onset_in_seconds: Union[int, float],
        duration_in_seconds: Union[int, float],
        description: str,
        str_format: str = "utf_8",
    ) -> int:
        """
        ...
        if str_format == "utf_8":
            ...
            if duration_in_seconds >= 0:
                ...
                return write_annotation_latin1(
                    self.handle,
                    np.round(onset_in_seconds * 10000).astype(np.int64),
                    np.round(duration_in_seconds * 10000).astype(int),
    ...
    def get_smp_per_record(self, ch_idx: int) -> int:
        ...
        if not np.isclose(np.round(smp_per_record), np.round(smp_per_record, 6)):
            warnings.warn(
                f"Sample frequency {fs} can not be represented accurately. \n"
