...
class TestEdfWriter(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.ch_info_bdf.update({"digital_max": 8388607, "digital_min": -8388608})
    ...
    def test_exceptions_raised(self):
        ...
        f = pyedflib.EdfWriter(
            self.edfplus_data_file, n_channels, file_type=pyedflib.FILETYPE_EDFPLUS
        )
    ...
    def test_write_functions(self):
        ...
        for file_type in [0, 1, 2, 3]:
            ...
            with pyedflib.EdfWriter(filename, 2, file_type=file_type) as f:
                ...
                for i in range(2):
                    ...
                    if res < 0:
                        print(
                            res,
                            f"Error for filetype {file_type} on writePhysicalSamples signal {i}",
                        )
                        error = True
                for i in range(2):
                    ...
                    if res < 0:
                        print(
                            res,
                            f"Error for filetype {file_type} on writeDigitalSamples signal {i}",
                        )
                        error = True
                ...
                if res < 0:
                    print(
                        res,
                        f"Error for filetype {file_type} on blockWritePhysicalSamples signal {i}",
                    )
                    error = True
                ...
                if res < 0:
                    print(
                        res,
                        f"Error for filetype {file_type} on blockWriteDigitalSamples signal {i}",
                    )
                    error = True
            ...
            with pyedflib.EdfReader(filename) as f:
                ...
                try:
                    ...
                    self.assertEqual(
                        data1.sum(),
                        data.sum() * 4,
                        "data written is not equal to data read",
                    )
                    self.assertEqual(len(data1), 400, "didn't write 400 samples")
                except Exception as e:
                    print(e)
                    error = True
    ...
    def test_subsecond_starttime(self):
        f = pyedflib.EdfWriter(
            self.edfplus_data_file, 1, file_type=pyedflib.FILETYPE_EDFPLUS
        )
    ...
    def test_subsecond_annotation(self):
        f = pyedflib.EdfWriter(
            self.bdfplus_data_file, 1, file_type=pyedflib.FILETYPE_BDFPLUS
        )
    ...
    def test_EdfWriter_BDFplus(self):
        f = pyedflib.EdfWriter(
            self.bdfplus_data_file, 2, file_type=pyedflib.FILETYPE_BDFPLUS
        )
        f.setSignalHeader(0, self.ch_info_bdf)
    ...
    def test_EdfWriter_BDF(self):
        f = pyedflib.EdfWriter(self.bdf_data_file, 2, file_type=pyedflib.FILETYPE_BDF)
        f.setSignalHeader(0, self.ch_info_bdf)
        f.setSignalHeader(1, self.ch_info_bdf)
    ...
    def test_EdfWriter_EDF(self):
        f = pyedflib.EdfWriter(self.edf_data_file, 2, file_type=pyedflib.FILETYPE_EDF)
        f.setSignalHeader(0, self.ch_info_edf)
    ...
    def test_EdfWriter_EDF_contextmanager(self):
        with pyedflib.EdfWriter(
            self.edf_data_file, 2, file_type=pyedflib.FILETYPE_EDF
        ) as f:
            f.setSignalHeader(0, self.ch_info_edf)
            f.setSignalHeader(1, self.ch_info_edf)
            data = np.ones(100) * 0.1
    ...
    def test_SampleWriting_digital(self):
        ...
        f = pyedflib.EdfWriter(
            self.edfplus_data_file, 2, file_type=pyedflib.FILETYPE_EDFPLUS
        )
        f.setSignalHeader(0, channel_info1)
        f.setSignalHeader(1, channel_info2)
    ...
    def test_physical_range_inequality(self):
        ...
        channel_info2 = {
            "label": "test_label_zero",
            "dimension": "mV",
            "sample_frequency": 100,
            "physical_max": max(channel_data2),
            "physical_min": min(channel_data2),
            "digital_max": 8388607,
            "digital_min": -8388608,
            "prefilter": "pre1",
            "transducer": "trans1",
        }
        f = pyedflib.EdfWriter(self.edf_data_file, 2, file_type=pyedflib.FILETYPE_BDF)
        f.setSignalHeader(0, channel_info1)
        f.setSignalHeader(1, channel_info2)

        # Test that assertion fails
        self.assertRaises(
            AssertionError, f.writeSamples, [channel_data1, channel_data2]
        )
    ...
    def test_sex_setting_correctly(self):
        ...
        for sex, expected in sex_mapping.items():
            f = pyedflib.EdfWriter(
                self.edf_data_file, 2, file_type=pyedflib.FILETYPE_EDFPLUS
            )
            f.setSex(sex)
            f.setSignalHeader(0, channel_info1)
            f.setSignalHeader(1, channel_info2)
            ...
            np.testing.assert_equal(f.getSampleFrequency(0), 100)
            self.assertEqual(
                f.getSex(), expected, f"set {sex}, but f.getSex()!={expected}"
            )
            with self.assertWarns(DeprecationWarning):
                self.assertEqual(
                    f.getGender(), expected, f"set {sex}, but f.getGender()!={expected}"
                )
            del f
    ...
    def test_non_one_second_record_duration(self):
        ...
        f = pyedflib.EdfWriter(
            self.edf_data_file, channel_count, file_type=pyedflib.FILETYPE_EDF
        )
    ...
    def test_sample_rate_deprecation(self):
        ...
        with self.assertRaises(FutureWarning):
            f.setSignalHeaders(
                [
                    {"sample_rate": sample_rate, **base_signal_header(idx)}
                    for idx in range(channel_count)
                ]
            )
        del f
    ...
    def test_EdfWriter_more_than_80_chars(self):
        ...
        with self.assertWarns(UserWarning):
            with pyedflib.EdfWriter(
                self.edf_data_file, 1, file_type=pyedflib.FILETYPE_EDFPLUS
            ) as f:
                f.setHeader(header)
                f.setSignalHeader(0, self.ch_info_edf)
                data = np.ones(100) * 0.1
    ...
    def test_EdfWriter_too_long_headers(self):
        channel_info1 = {
            "label": "l" * 100,  # this should be too long
            "dimension": "d" * 100,
            "sample_frequency": 100,
        ...
        with self.assertWarns(UserWarning):
            with pyedflib.EdfWriter(
                self.edf_data_file, 1, file_type=pyedflib.FILETYPE_EDF
            ) as f:
                f.setSignalHeader(0, channel_info1)
                data = np.ones(100) * 0.1
                f.writePhysicalSamples(data)
    ...
    def test_EdfWriter_out_of_bounds_pmin_pmax_dmin_dmax(self):
        channel_info = {
            "label": "l",  # this should be too long
            "dimension": "d",
            "sample_frequency": 100,
        ...
        with self.assertRaises(ValueError):
            ...
            # now a warning should appear.
            with pyedflib.EdfWriter(
                self.edf_data_file, 1, file_type=pyedflib.FILETYPE_BDF
            ) as f:
                f.setSignalHeader(0, channel_info1)
                data = np.ones(100) * 0.1
                f.writePhysicalSamples(data)
    ...
    def test_EdfWriter_float_sample_frequency(self):
        ...
        f = pyedflib.EdfWriter(
            self.edfplus_data_file, len(sfreqs), file_type=pyedflib.FILETYPE_EDFPLUS
        )
    ...
    def test_write_annotations_long_long(self):
        ...
        with pyedflib.EdfReader(self.edf_data_file) as f:
            ...
            np.testing.assert_array_equal(
                annotations[0], np.arange(0, 342000, 3600 * 2)
            )
