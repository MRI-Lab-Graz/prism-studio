...
class TestEdfReader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.bdf_data_file_datarec_2 = os.path.join(
            data_dir, "test_generator_datarec_generator_2.bdf"
        )
        cls.bdf_data_file_datarec_0_5 = os.path.join(
            data_dir, "test_generator_datarec_generator_0_5.bdf"
        )
    ...
    def test_EdfReader_BDF(self):
        ...
        for i in np.arange(f.signals_in_file):
            np.testing.assert_almost_equal(
                f.getSampleFrequencies()[i], sample_frequencies[i]
            )
            np.testing.assert_almost_equal(
                f.getSampleFrequency(i), sample_frequencies[i]
            )
            np.testing.assert_equal(
                f.getNSamples()[i], int(sample_frequencies[i] * datarecords)
            )
            np.testing.assert_almost_equal(
                f.getSignalHeader(i)["sample_frequency"], sample_frequencies[i]
            )
            np.testing.assert_almost_equal(
                f.getSignalHeaders()[i]["sample_frequency"], sample_frequencies[i]
            )
    ...
    def test_EdfReader_BDF_datarec_0_5(self):
        ...
        np.testing.assert_equal(
            f.datarecords_in_file, datarecords / datarecord_duration
        )
        np.testing.assert_equal(f.getFileDuration(), datarecords)
    ...
    def test_EdfReader_Check_Size(self):
        ...
        f = pyedflib.EdfWriter(
            self.bdf_broken_file, 1, file_type=pyedflib.FILETYPE_BDFPLUS
        )
    ...
    def test_BdfReader_Read_accented_file(self):
        ...
        f = pyedflib.EdfWriter(
            self.bdf_accented_file, 1, file_type=pyedflib.FILETYPE_BDFPLUS
        )
        f.setSignalHeader(0, channel_info)
        f.setTechnician("tec1")
        data = np.ones(100) * 0.1
