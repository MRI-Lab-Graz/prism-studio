"""
ANC (Austrian NeuroCloud) Export Converter
...
- Optionally converts DataLad to Git LFS for ANC compatibility
- Validates dataset against ANC requirements
- Creates export package ready for ANC submission
...
class ANCExporter:
    ...
    def __init__(self, dataset_path: Path, output_path: Optional[Path] = None):
        """
        Initialize ANC exporter.
        
        Args:
            dataset_path: Path to PRISM dataset
            output_path: Output path for ANC export (default: dataset_path_anc)
        """
        self.dataset_path = Path(dataset_path)
        self.output_path = Path(output_path) if output_path else Path(f"{dataset_path}_anc_export")
    ...
    def export(
        ...
        """
        logger.info(f"Exporting PRISM dataset to ANC format: {self.output_path}")
        ...
        logger.info(f"✓ ANC export completed: {self.output_path}")
    ...
    def _copy_dataset_structure(self):
        ...
        for item in self.dataset_path.iterdir():
            if item.name.startswith('.') and item.name not in ['.bidsignore', '.gitignore']:
                continue
    ...
    def _extract_metadata(self) -> Dict[str, Any]:
        ...
        if desc_file.exists():
            with open(desc_file) as f:
                ...
                metadata["ETHICS_APPROVALS"] = ", ".join(desc.get("EthicsApprovals", []))
                ...
                if "Authors" in desc and desc["Authors"]:
                    ...
                    if isinstance(first_author, str):
                        ...
                        metadata["AUTHOR_FAMILY_NAME"] = parts[-1] if len(parts) > 1 else "Author"
    ...
    def _generate_readme(self, metadata: Dict[str, Any]):
        ...
        # Load template
        template_path = Path(__file__).parent.parent.parent / "official" / "anc_templates" / "dataset_README_template.md"
        ...
        defaults = {
            ...
            "DATASET_DESCRIPTION": "A PRISM/BIDS dataset exported for ANC.",
            ...
            "MISSING_FILES_TABLE": "| | |",
            "KNOWN_ISSUES_TABLE": "| | |",
            "ADDITIONAL_NOTES": "This dataset was created using PRISM Studio and exported for ANC submission.",
...
This dataset was generated with PRISM Studio and exported for ANC (Austrian NeuroCloud) submission.
...
# For ANC (Austrian NeuroCloud) submission
...
"""
    ...
    def _convert_to_git_lfs(self):
        ...
        gitattributes = """# Git LFS configuration for ANC
...
3. Add files and commit:
   ```bash
   git add .
   git commit -m "Initial commit for ANC submission"
   ```

4. Add ANC remote and push:
   ```bash
...
""")
        logger.info(f"✓ Created Git LFS setup instructions")
...
**For ANC submission**: If ANC requires Git LFS instead of DataLad, run the export again with:
```bash
...
""")
        logger.info(f"✓ Added DataLad note")
    
    def _add_ci_examples(self):
        """Add CI/CD example files to export."""
        logger.info("Adding CI/CD example files...")
        
        templates_dir = Path(__file__).parent.parent.parent / "official" / "anc_templates"
...
""")
        logger.info(f"✓ Added CI setup instructions")
    
    def _validate_anc_requirements(self) -> Dict[str, Any]:
        """Validate export against ANC requirements."""
        logger.info("Validating ANC requirements...")
        ...
        if report["valid"]:
            logger.info("✓ All ANC requirements met")
        else:
...
def main():
    ...
    parser.add_argument(
        "-o", "--output",
        type=Path,
        help="Output path for ANC export (default: <dataset>_anc_export)"
    )
    parser.add_argument(
        "--git-lfs",
        action="store_true",
    ...
    print(f"\n✓ ANC export completed: {output_path}")
    print("\nNext steps:")
