"""Recipe authoring backend (Studio Recipe Builder + `prism_tools.py recipes save`).

Discovers survey/biometrics templates in a project (and optionally the official
library), extracts their items/ranges, and validates + writes recipe JSON files.
Keep Flask out of this module: callers resolve the official library root and
pass it in as ``global_root``.
"""
...
def extract_items_from_template(json_path: str, *, modality: str) -> list[str]:
...
def extract_item_ranges_from_template(json_path: str, *, modality: str) -> dict:
...
def extract_template_reversed_items(json_path: str, *, modality: str) -> list[str]:
...
def extract_items_missing_ranges_from_template(
    json_path: str, *, modality: str
) -> list[str]:
...
def recipe_output_path(dataset_path: str, *, modality: str) -> Path:
...
def save_recipe_to_project(
    ...
    *,
    ...
    known_items = (
        set(extract_items_from_template(matched_template["full_path"], modality=modality))
        or None
    )
    ...
    out_dir = recipe_output_path(dataset_path, modality=modality)
