...
class ParticipantsConverter:
    ...
    def _collapse_to_bids_participants_table(
        ...
        BIDS requires participants.tsv to contain one and only one row per
        participant. Session/run belong to session- or file-level structures, so
        they are removed here before collapsing repeated participant rows.
        """
