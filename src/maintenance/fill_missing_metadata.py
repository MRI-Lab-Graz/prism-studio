    ...
    try:
        ...
        if "additionalProperties" in schema and isinstance(schema["additionalProperties"], dict):
            ...
            if "properties" in item_schema:
                reserved = ["Technical", "Study", "Metadata", "I18n", "Scoring", "Normative", "Questions"]
                for key in data:
...
def main():
    parser = argparse.ArgumentParser(description="Fill missing metadata keys from schema")
    parser.add_argument("--modality", required=True, help="Modality (survey, biometrics, etc.)")
    parser.add_argument("--version", default="stable", help="Schema version (default: stable)")
    ...
    if not schema:
        print(f"Error: Could not load schema for {args.modality} (version: {args.version})")
        sys.exit(1)
