...
def fix_numeric_columns(tsv_path: str | Path, output_path: str | Path = None, dry_run: bool = False):
    """
    ...
    try:
        ...
        for col in df.columns:
            ...
            if col_lower in numeric_columns or any(keyword in col_lower for keyword in ['age', 'year', 'duration', 'score', 'response_time', 'rt']):
                try:
                    ...
                    if numeric_count > 0 and (numeric_count / non_null_count) > 0.5:
                        ...
                        print(f"✓ Fixed '{col}': converted {numeric_count} values to numeric")
...
def main():
    ...
    parser.add_argument('-o', '--output', help='Output path (default: overwrites input)')
    parser.add_argument('-n', '--dry-run', action='store_true', help='Show what would change without modifying')
