...
function _isValidOrcidFormat(value) {
    ...
    return /^(https?:\/\/orcid\.org\/)?\d{4}-\d{4}-\d{4}-\d{3}[\dX]\/?$/i.test(raw);
}

function _extractOrcidId(value) {
    const raw = String(value || '').trim();
    if (!raw) return '';
    ...
    const cleaned = raw
        ...
        .replace(/\/+$/, '');
    const match = cleaned.match(/^(?:https?:\/\/orcid\.org\/)?(\d{4}-\d{4}-\d{4}-\d{3}[\dX])$/i);
    if (!match) return '';
...
function _normalizeOrcid(value) {
    const raw = String(value || '').trim();
    if (!raw) return '';
    const orcidId = _extractOrcidId(raw);
    if (!orcidId) return raw;
...
function _applyOrcidCandidateToAuthorRow(row, candidate) {
    ...
    const firstInput = row.querySelector('.author-first');
    ...
    if (lastInput && !String(lastInput.value || '').trim() && candidate.family_name) {
        lastInput.value = String(candidate.family_name || '').trim();
    }
    if (orcidInput) {
        orcidInput.value = _normalizeOrcid(candidate.orcid || candidate.orcid_id || '');
    }
...
function _updateAuthorRowLabels() {
    ...
    rows.forEach((row, index) => {
        ...
        if (orcidLookupBtn) {
            orcidLookupBtn.title = 'Search ORCID by author name';
            orcidLookupBtn.setAttribute('aria-label', 'Search ORCID by author name');
        }
    });
    _refreshAuthorMoveButtons();
}
        ...
        <div class="row g-2">
            ...
            <div class="col-md-6">
                <div class="input-group input-group-sm" title="ORCID identifier (optional)">
                    ...
                    <button type="button" class="btn btn-outline-secondary author-orcid-find" title="Search ORCID by author name" aria-label="Search ORCID by author name">
