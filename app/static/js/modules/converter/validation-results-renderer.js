    ...
    if (validation.formatted) {
            ...
            formatted.errors.forEach((group) => {
                detailsHtml += `
                    <div class="validation-group mb-3 p-3 border rounded bg-light shadow-sm">
                        ...
                        ${renderValidationGroupFiles(group, escapeHtml)}
                    </div>
                `;
            });
        }
        ...
        if (errors.length > 0) {
            ...
            errors.forEach((errorMessage) => {
                detailsHtml += `<li class="text-danger small"><i class="fas fa-times me-1"></i>${escapeHtml(errorMessage)}</li>`;
            });
            detailsHtml += '</ul>';
        }
        if (warnings.length > 0) {
            ...
            warnings.forEach((warningMessage) => {
                detailsHtml += `<li class="text-warning small"><i class="fas fa-exclamation me-1"></i>${escapeHtml(warningMessage)}</li>`;
            });
            detailsHtml += '</ul>';
        }
    }
