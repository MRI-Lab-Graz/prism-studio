/**
 * LimeSurvey Quick Import Module
 * Handles .lss/.lsa file conversion with three export modes: combined, groups, questions
 * 
 * Exported function: initLimeSurveyQuickImport(elements)
 * Takes a dictionary of DOM elements and initializes all event listeners
 */

export function initLimeSurveyQuickImport(elements) {
    const {
        lssFile,
        lssTaskName,
        lssConvertBtn,
        ...
        lssTotalQuestions,
        lssDownloadAllBtn,
        lssQuestionnaireList,
        lssResultQuestions,
        lssIndividualCount,
        lssDownloadAllQuestionsBtn,
        lssQuestionsList,
        lssSaveGroupsToProjectBtn,
        lssSaveQuestionsToProjectBtn,
        lssSaveSuccess,
        lssSaveSuccessMessage
    } = elements;
            ...
            try {
                ...
                if (data.success) {
                    if (data.mode === 'questions') {
                        ...
                        if (lssIndividualCount) lssIndividualCount.textContent = `${data.question_count} question templates`;
                        ...
                        if (lssGroupCount) lssGroupCount.textContent = `${data.questionnaire_count} questionnaires`;
                        if (lssTotalQuestions) lssTotalQuestions.textContent = `${data.total_questions} total questions`;
                        ...
                        if (lssQuestionCount) lssQuestionCount.textContent = `${data.question_count} questions`;
                        if (lssPreviewContent) lssPreviewContent.textContent = JSON.stringify(lssConvertedData, null, 2);
    ...
    async function saveTemplatesToProject(templates) {
        try {
            ...
            if (data.success) {
                if (lssSaveSuccessMessage) lssSaveSuccessMessage.textContent = `Saved ${data.saved_count} template(s) to ${data.library_path}`;
