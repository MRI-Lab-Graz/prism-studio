...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const sharedApiModuleUrl = new URL('./shared/api.js', recipesScriptUrl).href;
  ...
  if (!derivFormat || !derivSurvey || !derivSessionsAll || !derivSessions || !derivRunBtn
    ...
    || !derivLayout || !derivLang || !derivIncludeRaw || !derivIncludeRecipePrefix || !derivMergeSeparate || !derivMergeCombined
  ...
  const RECIPES_PREF_KEYS = {
    ...
    include_recipe_prefix: 'derivIncludeRecipePrefix',
  ...
  function loadRecipesPreferences() {
    ...
    return fetchWithApiFallback(`/api/projects/preferences/recipes?project_path=${encodeURIComponent(requestProjectPath)}`)
      ...
      .then(data => {
        ...
        if (typeof prefs.include_recipe_prefix === 'boolean') {
          derivIncludeRecipePrefix.checked = prefs.include_recipe_prefix;
        }
  ...
  function logToTerminal(msg, type = 'info') {
    ...
    if (!commandMatch || typeof commandMatch.index !== 'number') {
      line.textContent = `[${timestamp}] ${prefix}${rawMessage}\n`;
    } else {
      const commandPrefix = commandMatch[1];
      const commandText = commandMatch[2];
      const messagePrefix = rawMessage.slice(0, commandMatch.index) + commandPrefix;
      line.appendChild(document.createTextNode(`[${timestamp}] ${prefix}${messagePrefix}`));
