...
class PrismValidatorGUI:
    def __init__(self, root):
        ...
        if apply_prism_theme:
            ...
            style.configure(
                "SubHeader.TLabel", font=("Segoe UI", 11), foreground="#666666"
            )
            style.configure(
                "Section.TLabel",
        ...
        style.configure("List.TFrame", background=self.colors["input_bg"])
        style.configure(
            "List.TLabel", background=self.colors["input_bg"], foreground="#8e8e93"
        )
        style.configure(
            "List.TCheckbutton",
        ...
        try:
            ...
            if logo_path.exists():
                ...
                h_size = 60
                w_size = int(
                    (float(pil_image.size[0]) * float(h_size))
                    / float(pil_image.size[1])
                )
        ...
        ttk.Label(text_frame, text="Prism Validator", style="Header.TLabel").pack(
            anchor="w"
        )
        ttk.Label(
            text_frame,
            text="Psychological Dataset Validation & Tools",
    ...
    def setup_validator_tab(self):
        ...
        config_frame = ttk.LabelFrame(
            self.validator_tab, text="  Configuration  ", padding="20"
        )
        ...
        # Dataset Path
        ttk.Label(config_frame, text="Dataset Path:").grid(
        ...
        self.bids_check = ttk.Checkbutton(
            options_frame, text="Run BIDS Validator (Standard)", variable=self.bids_var
        )
        self.bids_check.pack(side="left")

        # Action Button
        self.run_btn = ttk.Button(
            self.validator_tab, text="Validate Dataset", command=self.start_validation
        )
        self.run_btn.pack(pady=10, ipadx=20, ipady=5)

        # Results Area
        results_frame = ttk.LabelFrame(
            self.validator_tab, text="  Validation Results  ", padding="15"
        )
    ...
    def setup_survey_tab(self):
        ...
        lib_frame = ttk.LabelFrame(
            self.survey_tab, text="  Survey Library  ", padding="20"
        )
        ...
        ttk.Button(btn_frame, text="Load Files", command=self.load_library_files).pack(
            side="left", padx=5
        )
        ...
        left_frame = ttk.LabelFrame(
            paned_window, text="  Available Questionnaires  ", padding="10"
        )
        paned_window.add(left_frame, weight=1)

        columns = ("filename", "questions")
        self.survey_tree = ttk.Treeview(
        ...
        scrollbar_left = ttk.Scrollbar(
            left_frame, orient="vertical", command=self.survey_tree.yview
        )
        ...
        right_frame = ttk.LabelFrame(
            paned_window, text="  Select Questions  ", padding="10"
        )
        ...
        ttk.Button(
            ...
            text="Select All",
            style="Secondary.TButton",
            command=self.select_all_questions,
        ).pack(side="left", padx=(0, 5))
        ttk.Button(
            q_toolbar,
            text="Deselect All",
            style="Secondary.TButton",
            command=self.deselect_all_questions,
        ).pack(side="left")

        self.matrix_var = tk.BooleanVar(value=False)
        self.matrix_check = ttk.Checkbutton(
            q_toolbar,
            text="Group as Matrix",
            variable=self.matrix_var,
            command=self.on_matrix_change,
        )
        ...
        self.q_tree = ttk.Treeview(
            list_container, columns=columns, show="headings", selectmode="browse"
        )
        ...
        q_scrollbar = ttk.Scrollbar(
            list_container, orient="vertical", command=self.q_tree.yview
        )
        ...
        ttk.Button(
            action_frame,
            text="Export Selected to LimeSurvey (.lss)",
            command=self.export_lss,
        ).pack(side="right", ipadx=20, ipady=5)

        # --- Landgig: Survey Convert (Excel -> PRISM dataset) ---
        convert_frame = ttk.LabelFrame(
            self.survey_tab,
            text="  Landgig: Convert Excel to PRISM Dataset  ",
            padding="20",
        )
        ...
        ttk.Label(convert_frame, text="Excel File (.xlsx):").grid(
        ...
        ttk.Button(
            ...
            text="Browse...",
            style="Secondary.TButton",
            command=self.browse_convert_excel,
        ).grid(row=0, column=2, padx=5)

        ttk.Label(convert_frame, text="Survey Library Folder:").grid(
        ...
        ttk.Button(
            ...
            text="Browse...",
            style="Secondary.TButton",
            command=self.browse_convert_library,
        ).grid(row=1, column=2, padx=5)

        ttk.Label(convert_frame, text="Output Folder:").grid(
        ...
        ttk.Label(convert_frame, text="Dataset Name (optional):").grid(
        ...
        force_check = ttk.Checkbutton(
            convert_frame,
            text="Allow overwrite / non-empty output",
            variable=self.convert_force_var,
        )
        force_check.grid(row=4, column=1, sticky="w", padx=10, pady=(6, 0))

        self.convert_btn = ttk.Button(
            convert_frame,
            text="Convert Excel → PRISM Dataset",
            command=self.start_survey_convert,
        )
    ...
    def setup_recipes_tab(self):
        ...
        ttk.Label(frame, text="PRISM Dataset Path:").grid(
            row=0, column=0, sticky="w", pady=6
        )
        ...
        ttk.Button(frame, text="Browse...", command=self.browse_deriv_dataset).grid(
        ...
        modality_combo = ttk.Combobox(
            frame,
            textvariable=self.deriv_modality_var,
            values=("survey", "biometrics", "physio"),
            state="readonly",
        )
        ...
        ttk.Label(frame, text="Recipe Filter (optional):").grid(
            row=3, column=0, sticky="w", pady=6
        )
    ...
    def start_validation(self):
        ...
        thread = threading.Thread(
            target=self.run_validation_thread, args=(dataset_path,)
        )
        thread.daemon = True
        thread.start()
    ...
    def run_validation_thread(self, dataset_path):
        try:
            ...
            if validate_dataset:
                ...
                self.root.after(
                    0, lambda: self.display_error("Validator module not loaded")
                )
        except Exception as e:
            self.root.after(0, lambda e=e: self.display_error(str(e)))
    ...
    def build_cli_like_report(self, dataset_path, issues, stats):
        ...
        lines.append(
            f"Subjects: {len(stats.subjects)} | Sessions: {len(stats.sessions)} | Tasks: {len(stats.tasks)}"
        )
        ...
        try:
                        ...
                        with open(full_path, "r") as jf:
                            ...
                            if "Questions" in data and isinstance(
                                ...
                                for k, v in data["Questions"].items():
                                    ...
                                    q_levels = (
                                        v.get("Levels") if isinstance(v, dict) else None
                                    )
                                    q_choices = (
                                        v.get("Options")
                                        if isinstance(v, dict)
            ...
            for f in files:
                self.survey_tree.insert(
                    "", "end", values=(f[0], f[1]), tags=(f[0],)
                )  # Store filename in tags
        ...
        try:
            ...
            self.root.after(
                ...
                lambda: messagebox.showinfo(
                    "Success", f"PRISM dataset written to:\n{target}"
                ),
            )
        except Exception as e:
            self.root.after(
    ...
    def _recipes_thread(self, dataset_path, modality, out_format, survey_filter, lang, layout, include_raw, boilerplate):
        try:
            ...
            if validate_dataset:
                ...
                error_issues = [
                    i for i in (issues or []) if str(i[0]).upper() == "ERROR"
                ]
                if error_issues:
                    first = (
                        error_issues[0][1]
    ...
    def show_questions_for_survey(self, filename):
        ...
        for q in data["questions"]:
            ...
            self.q_tree.insert(
                "", "end", values=(check_mark, q_id, q_desc, q_scale), tags=(q_id,)
            )
    ...
    def toggle_question(self, item_id):
        ...
        current_state = self.selected_questions[self.current_survey_filename][
            "questions"
        ][q_id].get()
        new_state = not current_state
        self.selected_questions[self.current_survey_filename]["questions"][q_id].set(
            new_state
        )
    ...
    def on_matrix_change(self):
        if self.current_survey_filename:
            self.selected_questions[self.current_survey_filename]["matrix"].set(
                self.matrix_var.get()
            )

    def select_all_questions(self):
        if self.current_survey_filename:
            for var in self.selected_questions[self.current_survey_filename][
                "questions"
            ].values():
                var.set(True)
    ...
    def export_lss(self):
        ...
        for filename, state in self.selected_questions.items():
            ...
            selected_q_ids = [
                qid for qid, var in state["questions"].items() if var.get()
            ]
        ...
        if not export_list:
            messagebox.showwarning(
                "Warning",
                "Please select at least one question from one survey to export.",
            )
            return

        save_path = filedialog.asksaveasfilename(
            defaultextension=".lss", filetypes=[("LimeSurvey Structure", "*.lss")]
        )
        if not save_path:
            return
        ...
        try:
            if generate_lss:
                ...
                messagebox.showinfo(
                    "Success", f"Exported {len(export_list)} surveys to:\n{save_path}"
                )
            else:
