...
def update_participants_tsv(
    dataset_root: Path, 
    participant_ids: set[str] | list[str],
    log_fn: Any = None
) -> None:
    ...
    if participants_tsv.exists():
        try:
            ...
            if "participant_id" not in header:
                if log_fn:
                    log_fn("[WARNING] participants.tsv exists but has no participant_id column. Will recreate.")
                existing_rows = []
                header = ["participant_id"]
            else:
                existing_ids = {row.get("participant_id", "").strip() for row in existing_rows if row.get("participant_id", "").strip()}
    ...
    if not new_ids:
        if log_fn:
            log_fn(f"[INFO] All {len(participant_ids)} participant IDs already in participants.tsv")
        return
    ...
    if log_fn:
        log_fn(f"[INFO] Updated participants.tsv: added {len(new_ids)} new participant(s)")
