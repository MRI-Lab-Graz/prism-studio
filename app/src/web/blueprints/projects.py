...
def datalad_clean_status():
    """Return whether the current project's DataLad working tree is clean."""
    current_project = get_current_project()
    project_path = str(current_project.get("path") or "").strip()
    if not project_path:
        return jsonify({"success": False, "error": "No current project loaded."}), 400

    result = _project_manager.check_datalad_clean_status(project_path)
    return jsonify({"success": True, "datalad_clean": result})
...
def get_sourcedata_files():
    ...
    Query params:
        kind: optional converter kind filter (survey, biometrics, environment,
              participants, physio, eyetracking, all). Defaults to survey.

    Returns files matching converter-supported extensions found in the
    sourcedata/ directory recursively (includes subdirectories such as
    wide_to_long/).
    """
