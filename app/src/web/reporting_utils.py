...
def format_validation_results(
    ...
    total_files = stats_total or (len(file_paths) if file_paths else len(valid_files) + len(invalid_files))
    ...
    if dataset_stats:
        if hasattr(dataset_stats, "subjects"):
            ...
            unique_sessions = {s.split("/", 1)[1] if "/" in s else s for s in session_entries if s}
            serializable_stats = {
