...
class ProjectManager:
    ...
    def _migrate_parent_tracked_directory_to_subdataset(
        ...
        # Deliberately a plain, path-scoped `git commit`, not `datalad save
        # --updated`: per DataLad's own docs, `--updated` "only saves
        # previously tracked paths" - but `git rm --cached` just moved this
        # exact path OUT of being tracked, so `--updated` doesn't consider it
        # in scope and silently commits some *other* unrelated pending
        # change instead (e.g. a previous subject's leftover subdataset
        # gitlink update), dropping this path's staged removal entirely.
        # A pathspec-limited `git commit` has no such ambiguity - it commits
        # exactly what's staged under this path, nothing else.
