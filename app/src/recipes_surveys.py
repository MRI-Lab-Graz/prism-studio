...
def _export_recipe_aggregated(
    ...
    if out_format == "csv":
        ...
        try:
            ...
            for col in df_sav.columns:
                # Replace "n/a" with NaN (no downcasting with future behavior enabled)
...
def _resolve_variant_for_path(
    ...
    *,
    ...
    if metadata is None and prism_root and task_name:
...
def compute_survey_recipes(
    *,
    ...
    for recipe_id, rec in sorted(recipes.items()):
        ...
        matching = []
        for p in tsv_files:
            task = _strip_acq_from_task(_extract_task_from_survey_filename(p))
            if task == survey_task:
                matching.append(p)
        if not matching:
            continue
        ...
        if out_format in ("csv", "xlsx", "save"):
            if merge_all:
                ...
                for in_path in matching:
                    ...
                    # (but skip if column already starts with the recipe prefix)
                    for row_index, score_row in enumerate(out_rows):
                        merged = {"participant_id": sub_id, "session": ses_id}
                        for col in out_header:
                            # Only add prefix if column doesn't already start with recipe_id
