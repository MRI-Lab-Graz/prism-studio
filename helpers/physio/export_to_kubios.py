...
def export_to_edf(df, output_file, fs, ecg_col, marker_col):
    ...
    ch1 = {
        ...
        "dimension": "uV",
        "sample_frequency": fs,
        "physical_max": df[ecg_col].max(),
        "physical_min": df[ecg_col].min(),
        "digital_max": 32767,
        "digital_min": -32768,
        "transducer": "AgAgCl",
        "prefilter": "",
    ...
    try:
        f = pyedflib.EdfWriter(
            output_file, n_channels, file_type=pyedflib.FILETYPE_EDFPLUS
        )
...
def export_to_mat(df, output_file, fs, ecg_col, marker_col):
    ...
    mat_dict = {"data": df[ecg_col].values.reshape(-1, 1), "fs": float(fs)}
...
def export_to_kubios(bids_tsv, output_file=None, fmt="dat"):
    ...
    try:
        df = pd.read_csv(
            bids_tsv, sep="\t", compression="gzip" if bids_tsv.endswith(".gz") else None
        )
    except Exception as e:
    ...
    ecg_col = next(
        (c for c in df.columns if "EKG" in c.upper() or "ECG" in c.upper()), None
    )
    marker_col = next(
        (c for c in df.columns if "MARKER" in c.upper() or "TRG" in c.upper()), None
    )
    ...
    if fmt == "edf":
        ...
        print(
            "3. Select 'Custom ASCII' as file type if needed, or just select the file."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--format",
        choices=["dat", "edf", "mat", "csv"],
        default="dat",
        help="Output format: 'dat'/'csv' (ASCII), 'edf' (EDF+), or 'mat' (Kubios MAT)",
    )
