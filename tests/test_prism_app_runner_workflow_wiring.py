...
PRISM_APP_RUNNER_HANDLERS = REPO_ROOT / "app" / "src" / "web" / "blueprints" / "tools_prism_app_runner_handlers.py"
...
class TestPrismAppRunnerWorkflowWiring(unittest.TestCase):
    def test_prism_app_runner_template_renders_disabled_html_state(self):
        ...
        self.assertIn('<fieldset{% if prism_app_runner_disabled %} disabled aria-disabled="true"{% endif %}>', template_content)
    ...
    def test_prism_app_runner_page_handler_no_longer_returns_disabled_json(self):
        ...
        self.assertIn('prism_app_runner_disabled=not PRISM_APP_RUNNER_ENABLED,', handler_content)
        self.assertIn('prism_app_runner_disabled_message=PRISM_APP_RUNNER_DISABLED_MESSAGE,', handler_content)
