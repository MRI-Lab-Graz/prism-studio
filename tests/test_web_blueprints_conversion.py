...
class TestSurveyConvertValidateEndpoint(unittest.TestCase):
    ...
    def test_validate_merges_selected_tasks_filter(self):
        import importlib
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            survey_dir.mkdir(parents=True, exist_ok=True)
            (survey_dir / "survey-pss.json").write_text("{}", encoding="utf-8")
            (survey_dir / "survey-gad.json").write_text("{}", encoding="utf-8")

            captured_survey_filters = []

            def fake_run_survey_with_official_fallback(_converter, **kwargs):
                captured_survey_filters.append(kwargs.get("survey"))
        ...
        self.assertGreaterEqual(len(captured_survey_filters), 2)
        self.assertTrue(
            all(str(value or "") == "pss" for value in captured_survey_filters)
        )
    ...
    def test_validate_removes_stale_tool_limesurvey_sidecars_for_non_lsa(self):
        import importlib
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            project_root = tmp_path / "project"
            project_root.mkdir(parents=True, exist_ok=True)
            survey_dir = project_root / "rawdata" / "sub-001" / "ses-1" / "survey"
            survey_dir.mkdir(parents=True, exist_ok=True)
            stale_tool_json = survey_dir / "sub-001_ses-1_tool-limesurvey_survey.json"
            stale_tool_json.write_text("{}", encoding="utf-8")

            library_root = tmp_path / "library"
            survey_library = library_root / "survey"
            survey_library.mkdir(parents=True, exist_ok=True)
            (survey_library / "survey-pss.json").write_text("{}", encoding="utf-8")
            ...
            def fake_run_survey_with_official_fallback(_converter, **kwargs):
                ...
                if not kwargs.get("dry_run"):
                    out_file = (
                        ...
                        / "ses-1"
                        / "survey"
                        / "sub-001_ses-1_task-pss_events.tsv"
                    )
        ...
        payload = response.get_json()
        self.assertTrue(payload.get("success"))
        self.assertFalse(stale_tool_json.exists())
...
class TestSurveyOfficialTemplateCopy(unittest.TestCase):
    ...
    def test_api_survey_convert_registers_detected_sessions_for_all(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch.object(
                    ...
                    return_value=SimpleNamespace(
                        tasks_included=["wellbeing"],
                        detected_sessions=["1", "2"],
                    ),
                ),
                patch.object(
                    handlers,
                    "_extract_tasks_from_output",
                    return_value=["wellbeing"],
                ),
                patch.object(
                    handlers,
            ...
            registered_sessions = [call.args[1] for call in register_mock.call_args_list]
            self.assertEqual(registered_sessions, ["1", "2"])
...
class TestParticipantsPreviewApiEdgeCases(unittest.TestCase):
    ...
    def test_convert_returns_409_when_existing_participant_files_without_force_overwrite(
        ...
        participants_json.write_text(
            json.dumps({"participant_id": {"Description": "old"}}),
            encoding="utf-8",
        )
        ...
        try:
            ...
            self.assertEqual(response.status_code, 409)
            payload = response.get_json() or {}
            self.assertIn("already exist", payload.get("error", ""))
            self.assertEqual(
                participants_tsv.read_text(encoding="utf-8"),
                "participant_id\tage\nsub-old\t99\n",
            )
            self.assertEqual(
                json.loads(participants_json.read_text(encoding="utf-8")),
                {"participant_id": {"Description": "old"}},
            )
        finally:
            if old_detect is not None:
                id_detection_module.detect_id_column = old_detect
    ...
    def test_convert_force_overwrite_replaces_existing_participant_files_and_reports_location(
        ...
        try:
            ...
            self.assertEqual(
                set(payload.get("overwritten_files") or []),
                {str(participants_tsv), str(participants_json)},
            )

            out_df = pd.read_csv(participants_tsv, sep="\t")
            self.assertEqual(list(out_df.columns), ["participant_id", "age"])
            self.assertEqual(out_df["participant_id"].tolist(), ["sub-001", "sub-002"])

            out_json = json.loads(participants_json.read_text(encoding="utf-8"))
            self.assertIn("participant_id", out_json)
            self.assertEqual(
                out_json["participant_id"].get("Description"),
...
class TestParticipantsInputFileEdgeCases(unittest.TestCase):
    ...
    @patch.object(participants_module, "resolve_effective_library_path")
    def test_preview_collapses_long_rows_to_unique_participants(
        self,
        mock_resolve_library,
        _mock_template_ids,
    ):
        ...
        try:
            ...
            rows = payload.get("preview_rows") or []
            self.assertEqual(
                [row.get("participant_id") for row in rows], ["sub-001", "sub-002"]
            )
            self.assertNotIn("session", payload.get("columns") or [])
        finally:
            self._restore_id_detection(id_detection_module, old_detect, old_has_pm)
...
class TestParticipantsConverterInputEdgeCases(unittest.TestCase):
    ...
    def test_convert_collapses_duplicate_normalized_participant_ids(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(participant_ids, ["sub-001", "sub-1"])
