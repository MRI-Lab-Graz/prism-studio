...
class TestRecipeBuilderWorkflowWiring(unittest.TestCase):
    def test_recipe_builder_template_links_to_projects_page_when_no_project_loaded(self):
    ...
    def test_recipe_builder_script_uses_api_fallback_for_load_and_save_requests(self):
        ...
        self.assertIn("'/api/recipe-builder/surveys?dataset_path=' + encodeURIComponent(path) + includeGlobal", content)
        self.assertIn("'/api/recipe-builder/items?task=' + encodeURIComponent(task) +", content)
        self.assertIn("'/api/recipe-builder/load?task=' + encodeURIComponent(task) +", content)
        self.assertIn("const response = await fetchWithApiFallback('/api/recipe-builder/save', {", content)
