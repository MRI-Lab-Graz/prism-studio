...
class TestWindowsFilenames:
    ...
    def test_reserved_names(self):
        ...
        print(f"    ℹ️  Note: Reserved names with compound extensions (.nii.gz)")
        print(f"        may not be detected by splitext - this is a known limitation")
    ...
    def test_trailing_spaces_dots(self):
        ...
        print(f"    ℹ️  Note: 'file .json' has space before extension (valid)")
    ...
    def test_filename_length(self):
        ...
        if short_issues:
            print(f"    ❌ 200 char filename should be valid")
            return False
        
        if not long_issues:
            print(f"    ❌ 260 char filename should be invalid")
            return False
        
        print(f"    ✅ Filename length validation working")
        return True
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for name, content in test_cases:
                try:
                    ...
                    if len(read_back) > 0:
                        print(f"    ✅ {name}: {len(content)} -> {len(read_back)} chars")
                    else:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if content1 != content2:
                    print(f"    ✅ File operations without lock conflicts")
                    return True
                else:
                    print(f"    ❌ File not updated properly")
                    return False
