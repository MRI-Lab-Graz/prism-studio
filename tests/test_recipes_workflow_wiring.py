...
class TestRecipesWorkflowWiring(unittest.TestCase):
    def test_recipes_page_uses_explicit_project_paths_and_api_fallback(self):
        ...
        self.assertIn("window.currentProjectPath = {{ (current_project.path or '') | tojson }};", template_content)
        self.assertIn("async function fetchWithApiFallback(", script_content)
        self.assertIn("fetchWithApiFallback(`/api/recipes-modalities?dataset_path=${encodeURIComponent(requestProjectPath)}`)", script_content)
        self.assertIn("fetchWithApiFallback(`/api/recipes-sessions?dataset_path=${encodeURIComponent(requestProjectPath)}`)", script_content)
        self.assertIn("fetchWithApiFallback(`/api/projects/preferences/recipes?project_path=${encodeURIComponent(requestProjectPath)}`)", script_content)
        self.assertIn("body: JSON.stringify({ project_path: requestProjectPath, preferences: prefs }),", script_content)
        self.assertIn("const response = await fetchWithApiFallback('/api/recipes-surveys', {", script_content)
    ...
    def test_recipes_page_resets_state_and_ignores_stale_project_switch_responses(self):
        ...
        self.assertIn("return requestToken === activeToken && requestProjectPath === resolveProjectPath();", content)
        self.assertIn("return runToken === recipeRunToken && requestProjectPath === resolveProjectPath();", content)
