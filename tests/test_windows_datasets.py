...
class TestWindowsDatasetStructure:
    ...
    def test_case_insensitive_validation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if sys.platform.startswith("win"):
                ...
                print(f"    Created {len(files)} names, found {len(actual_files)} files")
                
                if len(actual_files) == 1:
                    print(f"    ✅ Windows case-insensitive behavior confirmed")
                else:
                    print(f"    ⚠️  Expected 1 file due to case-insensitivity")
            else:
                print(f"    ⚠️  Not on Windows, case-sensitive behavior")
    ...
    def test_mixed_case_subject_folders(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            actual_dirs = [d for d in os.listdir(tmpdir) if os.path.isdir(os.path.join(tmpdir, d))]
            ...
            if sys.platform.startswith("win") and len(actual_dirs) == 1:
                print(f"    ✅ Case-insensitive folder creation (expected on Windows)")
            elif not sys.platform.startswith("win") and len(actual_dirs) == len(folders):
                print(f"    ✅ Case-sensitive folder creation (expected on Unix)")
    ...
    def test_windows_path_validation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if backslash_paths:
                ...
                print(f"    ✅ All paths normalized to forward slashes")
...
class TestWindowsFileHandling:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(test_file, 'r') as f:
                ...
                try:
                    ...
                    print(f"    ✅ Can read file while open for reading")
                except Exception as e:
            ...
            try:
                ...
                print(f"    ✅ Can write after file closed")
            except Exception as e:
    ...
    def test_long_filename_paths(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if path_length > 260:
                print(f"    ⚠️  Exceeds traditional Windows limit (260)")
            ...
            try:
                ...
                if os.path.exists(full_path):
                    print(f"    ✅ Long path handled successfully")
                    
                    # Try to read it back
                    content = CrossPlatformFile.read_text(full_path)
                    print(f"    ✅ Can read long path")
                else:
                ...
                if sys.platform.startswith("win"):
                    print(f"    ℹ️  May need long path support enabled in Windows")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if sys.platform.startswith("win"):
                ...
                try:
                    ...
                    print(f"    ✅ Can read read-only file")
                except Exception as e:
                ...
                try:
                    CrossPlatformFile.write_text(test_file, '{"test": "updated"}')
                    print(f"    ⚠️  Write to read-only succeeded (unexpected)")
                except Exception as e:
                ...
                print(f"    ⚠️  Not on Windows, skipping")
...
"""
            CrossPlatformFile.write_text(
                os.path.join(tmpdir, ".bidsignore"),
                bidsignore_content
            )
            ...
            print(f"    ✅ Created test structure with .bidsignore")
    ...
    def test_cross_platform_dataset_sharing(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if len(filtered) == 0:
                print(f"    ✅ All cross-platform system files filtered")
                return True
            else:
