...
class TestTemplateEditorWorkflowWiring(unittest.TestCase):
    ...
    def test_template_editor_project_switch_invalidates_project_bound_actions(self):
        ...
        self.assertIn("function handleProjectContextChange(previousProjectPath, nextProjectPath) {", script_content)
        ...
        self.assertIn("window.addEventListener('prism-project-changed', async () => {", script_content)
        self.assertIn("The editor kept the content as a detached draft", script_content)
        self.assertIn("Saved to the previous project library before the active project changed.", script_content)
        self.assertIn("request.args.get(\"project_path\")", blueprint_content)
        self.assertIn("explicit_project_path=payload.get(\"project_path\")", blueprint_content)

    def test_template_editor_schema_switch_refreshes_schema_aware_template_statuses(self):
        content = TEMPLATE_EDITOR_SCRIPT.read_text(encoding="utf-8")
    ...
    def test_template_editor_switch_cancel_and_failure_revert_select_state(self):
        ...
        self.assertIn("function revertTrackedSelectValue(selectEl, fallbackValue = '') {", content)
        self.assertIn("const previousModality = getTrackedSelectValue(modalityEl);", content)
        self.assertIn("revertTrackedSelectValue(modalityEl, previousModality);", content)
        self.assertIn("commitTrackedSelectValue(modalityEl);", content)
        self.assertIn("const previousSchemaVersion = getTrackedSelectValue(schemaEl);", content)
        self.assertIn("revertTrackedSelectValue(schemaEl, previousSchemaVersion);", content)
    ...
    def test_template_editor_failed_template_load_restores_previous_editor_state(self):
        ...
        self.assertIn("projectTemplateSelectEl.addEventListener('change', async () => {", content)
        self.assertIn("globalTemplateSelectEl.addEventListener('change', async () => {", content)
    ...
    def test_template_editor_blank_template_creation_is_guarded_and_rollback_safe(self):
        ...
        self.assertIn("if (hasUnsavedChanges() && !confirm('You have unsaved changes. Create a new blank template and discard them?')) {", content)
        self.assertIn("const previousEditorState = captureEditorState();", content)
        self.assertIn("restoreEditorState(previousEditorState);", content)
