...
class TestGitHubSigningConfiguration:
    ...
    def test_documentation_exists(self):
        ...
        try:
            ...
            found_keywords = sum(
                1 for kw in signing_keywords if kw.lower() in content.lower()
            )
    ...
    def test_signpath_action_version(self):
        ...
        try:
            ...
            match = re.search(
                r"signpath/github-action-submit-signing-request@(v[\d.]+)", content
            )
        ...
        try:
            ...
            if "signpath/github-action-submit-signing-request" in content:
                ...
                if "signing-policy-slug" in content:
                    match = re.search(
                        r"signing-policy-slug:\s*['\"]?([^'\"]+)['\"]?", content
                    )
                    if match:
            ...
            output_match = re.search(
                r"output-artifact-path:\s*['\"]?([^'\"]+)['\"]?", content
            )
            if output_match:
